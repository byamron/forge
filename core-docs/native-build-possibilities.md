...
> **Verification note.** Designer-side references in this document (detector names, trait signatures, file paths, phase numbers, event-type names like `Permission::Decided` or `LocalOps`) come from a single research pass over the Designer codebase. They were not individually re-verified during the writing of this document. Treat them as approximate. If you need to cite a specific Designer symbol or path in implementation work, re-read `/Users/benyamron/dev/designer` directly — names may have changed since the source pass, or may have been remembered imprecisely. The architectural patterns described here are the durable contribution of this document; the Designer specifics are illustrative.

This document captures architectural patterns that become available when a Noticed-like system is built natively into an AI coding app instead of shipped as a Claude Code plugin. It exists because a prototype called Designer-Noticed (inside the Designer app at `/Users/benyamron/dev/designer`, Phase 21.A2) explored most of these patterns before that project changed direction. The work is not lost — these are the ideas worth carrying forward whenever native integration becomes possible, either inside Claude Code itself or in another AI coding surface.

Noticed today is a plugin. Plugins read what Claude Code chooses to expose (session transcripts, hook events, settings) and write through user-approved diffs. That boundary is the source of Noticed's safety guarantees and the source of its limitations. The transferable Designer ideas — synthesis boundaries, content-addressed dedup, richer proposal kinds, signal events — are already covered in plan items P10-P14. This document covers what remains: the ideas that *require* owning the runtime.

Audience: future Noticed contributors evaluating a native re-platform, or anyone deciding whether to build a new learning-layer feature inside an AI coding app from scratch.

---

## Why this exists

A plugin lives outside the host's event loop. It can observe what the host serializes to disk (transcripts) and what the host invokes it for (hooks), but it cannot subscribe to fine-grained events, mutate the host's UI, or run analyses inside the same process that's about to make a decision. Designer-Noticed was a clean-slate rebuild of Noticed's value proposition inside a process that owns its own event store, UI, and runtime. The constraints that shaped Noticed (no shared state with Claude Code, only retrospective analysis, markdown-only outputs) dissolve at that boundary.

This doc is not a recommendation to re-platform. Plugin distribution is what makes Noticed usable today. It's a catalog of capability ceilings the plugin model imposes, so a future port has a reference for what to reclaim first.
...
- **Replay.** Re-run a detector across historical events to validate threshold changes against real data, no synthetic fixtures needed. Noticed today validates threshold changes against `tests/fixtures/` — Designer can validate against actual user history.
...
- **Time-windowed queries.** "How did approval-grant rate change in the 24h after we shipped scope rule X" is a single query. Noticed has to reconstruct this from transcript JSONL re-parsing.
- **Schema evolution.** Events are versioned per type. A detector v2 can choose to ignore v1 events or re-interpret them, without losing the v1 history. Noticed's `dismissed.json` is a flat structure with no versioning — schema changes require migration scripts.
...
A native Noticed would store events in SQLite-WAL or LMDB. JSONL works but loses query speed. Designer used a custom flat-file format with B-tree indices.

---

## Native detectors that need owned-runtime signals

Six of Designer's nine detectors overlap with Noticed (repeated_correction, repeated_prompt_opening, multi_step_tool_sequence, config_gap, domain_specific_in_claude_md, memory_promotion). Noticed already implements equivalents. The remaining three are inaccessible to a plugin because the underlying events are never exposed:
...
Required signal: per-invocation token counts attributed to the user (not the plugin's own consumption). Noticed can approximate this from transcript token estimates but with substantial error and a one-cycle lag (estimates come from the previous session).
...
This is the pattern P10 imports into Noticed. But the native version can do more than the plugin port:
...
Noticed's UI is the Claude Code terminal: a startup notification line, markdown proposal cards rendered through a skill, and a chat-based approval flow. Designer's UI is React (`packages/app/src/components/DesignerNoticed/`) with:
...
- **Settings → Activity → "Designer noticed" archive** — read-only history of every finding ever recorded, filterable, with full evidence trails. Noticed has nothing equivalent; `applied.json` and `dismissed.json` are not surfaced anywhere.
- **Snooze with explicit duration picker** — Noticed proposals can be skipped, but "remind me in a week" requires another UI affordance the terminal can't provide.
...
- **`RemovalCandidate`** (also in P12) — must type the filename to confirm. Noticed's P12 port implements this in markdown by asking the user to type the filename in chat; the native version is a confirmation modal with the actual file contents visible.
...
- **Synthesis humanization** — turn "detector emitted 5 findings with shared digest X" into a one-sentence title that reads like a human wrote it. Today Noticed uses the Claude API for this, charged against the user's quota.
...
The plugin version is constrained to the Claude API. The user's quota is the only model budget Noticed has access to, which is why the LLM quality gate (P0b) is the only LLM step in Noticed today and why P13 signals are recorded for *future* local retuning that has no host to run in.
...
In a plugin world this round-trip doesn't exist. Noticed writes a scoped rule with `paths` frontmatter; Claude Code's rule loader picks it up next session. There's no feedback to Noticed about whether the rule actually changed Claude's loading behavior. P9 (session health analysis) is Noticed's attempt to approximate this by detecting frequently-read files, but it operates blind to whether scoped rules it already produced are reducing re-reads.

A native build closes the loop: detector → proposal → applied artifact → observed prompt-structure change → measured reduction in re-reads → confidence update for the detector. Every step except the last exists in Noticed today; the last requires runtime instrumentation only the host can provide.
...
Noticed does the opposite: `dismissed.json` and `applied.json` are mutable JSON dicts updated in place. This works at Noticed's scale but loses three things:
...
- **Audit.** "When was this dismissed and why" requires Noticed to add fields to the dismissed entry. In Designer it's just `SELECT * FROM events WHERE proposal_id = X ORDER BY ts`.
- **Concurrent writes.** Two Noticed processes finalizing simultaneously can race on `dismissed.json`. The current `_write_json_atomic` helper handles the file write but not the read-modify-write sequence. An append-only log doesn't have this problem (each writer appends independently; the projection sorts by timestamp).

A native build standardizes on event sourcing for all interaction state. Noticed's `signals.jsonl` (P13) is a small step in this direction — append-only, replayable — but only for one slice of the data.

---

## Migration thought experiment

If Noticed moved native — either inside Claude Code's runtime or as a standalone AI coding surface — what stays and what changes:
...
- The LLM quality gate prompt (`noticed/agents/session-analyzer.md`). The synthesis step's prompt doesn't change with the runtime.
...
- Storage layout (`.claude/noticed/` + `~/.claude/noticed/projects/<hash>/`) → event log + projection cache. Split between project-shared and user-personal becomes "events have project_id + workspace_id".
...
**New (no equivalent in Noticed today):**
...
**Removed:**
- Plugin manifest and version-bump rules (`noticed/.claude-plugin/plugin.json` + `marketplace.json`). Native distribution uses the host's release pipeline.
- Path encoding for `~/.claude/projects/<hash>/` resolution. The runtime knows what project it's in.
- The Noticed-co-installation probe (P14). Subsumed by the host's single learning layer.
...
- **Synthesis boundaries** (P10): the single biggest UX improvement. Native or not, Noticed benefits from separating detection from surfacing.
