...
**Where we are (2026-04-04):** P0 validation complete on Noticed repo, portfolio-site, and PriorityAppXcode (v0.3.7). Key findings: raw proposal quality is poor without LLM gate (0-22% acceptance), memory promotions are uniformly bad (44% of proposals, all dismissed), and a new bug found where applied proposals reappear (no applied-ID filter in `build_proposals()`). LLM quality gate shipped but needs background-analyze.py to have run first. 419 tests, all passing.

**What to do next — start these workspaces:**

### Workspace 1: P0 validation (portfolio-site + PriorityAppXcode)
Run `/noticed` on each project, record outcomes, dismiss some proposals, run again to verify calibration (impact deflation, safety gate, skip decay). This is manual testing — run the command, interact with proposals, document results. Findings go into `core-docs/plan.md` under the P0 validation results section.

### Workspace 2: P1 design + implementation (ambient presence)
Resolve the open design questions first (see below), then implement. This is the biggest UX change — how Noticed communicates with the user between `/noticed` runs. **Depends on P0 validation insights** (what proposals look like on real projects informs how they should be surfaced proactively).
...
- **P3** (reliability + `/noticed --diagnose`) — can start in parallel but `diagnose.py` output depends on P1 settings design
...
- **"Quiet" mode is confusing.** "Only analyzes when you run `/noticed`" is ambiguous — does it mean no background prep, or no proposals presented? The user should understand exactly what changes when they change a setting.
- **Session count is the wrong trigger.** The unit of value is proposals, not sessions. 5 sessions with no new patterns shouldn't trigger anything. The trigger should be: "Noticed has new proposals ready" — not "enough sessions passed."
...
- Feedback loop calibration — need to dismiss proposals on portfolio-site, run `/noticed` again, verify impact deflation and safety gate activate

## Spec & Roadmap

Original spec (`core-docs/spec.md`) and roadmap (`core-docs/roadmap.md`) are checked in. Key deviations from spec:
- Three separate skills unified into `/noticed`
...
**Decision (2026-04-03):** Feedback data (dismissed.json, applied.json, feedback_signals in analyzer-stats) moves to `.claude/noticed/` in the repo (git-tracked, shared across contributors). Settings, cache, pending proposals, and session logs stay in `~/.claude/noticed/` (personal, per-machine).

| Data | Location | Why |
|---|---|---|
| `dismissed.json` | `.claude/noticed/` (repo) | Dismissals affect what all contributors see |
| `history/applied.json` | `.claude/noticed/` (repo) | Provenance for git-tracked artifacts |
| `feedback_signals.json` | `.claude/noticed/` (repo) | Calibration (impact deflation, safety gate) is project-level |
| `settings.json` | `~/.claude/noticed/projects/<hash>/` (user) | Personal preferences |
| `cache/` | `~/.claude/noticed/projects/<hash>/` (user) | Ephemeral, per-machine |
| `proposals/pending.json` | `~/.claude/noticed/projects/<hash>/` (user) | Regenerated each run |
| `unanalyzed-sessions.log` | `~/.claude/noticed/projects/<hash>/` (user) | Per-machine session tracking |

**Implementation:**
- Add `get_project_data_dir(root) -> Path` to `project_identity.py` — returns `<root>/.claude/noticed/`
- Update `finalize-proposals.py`: write `dismissed.json` and `history/applied.json` via `get_project_data_dir()`
- Extract `feedback_signals` from `analyzer-stats.json` into its own `feedback_signals.json` in `.claude/noticed/`. Leave legacy stats (correction/post_action counts, theme_outcomes) in `~/.claude/noticed/` for backward compat.
- Update `build-proposals.py`: read dismissed + applied + feedback_signals from `.claude/noticed/`
- Update `check-pending.py`: read dismissed from `.claude/noticed/`
...
**Goal:** Run `/noticed` on 3+ real projects, measure proposal acceptance rate, validate feedback loop calibration.
**Impacts:** Accuracy
...
1. **Run `/noticed` on portfolio-site**
...
2. **Run `/noticed` on PriorityAppXcode** — same process

3. **Run `/noticed` on 1-2 additional active projects** — same process

4. **Run `/noticed` again on each project** (second pass)
...
#### P0 validation results: Noticed repo (2026-04-03)
...
|------|-------|---------|--------|
| agent (generic workflows) | 5 | All dismissed | not_relevant — read→write→execute patterns, identical to Noticed repo findings |
...
3. **No LLM quality gate on first run** — neither project had a deep analysis cache, so users would see the full unfiltered proposal set on first `/noticed` run unless the synchronous fallback in SKILL.md Step 1b fires. This is by design (background-analyze.py runs on SessionStart), but means the first `/noticed` experience is the worst.
...
**Decision (2026-04-03):** The `analysis_depth` setting is removed. LLM quality gate is implicit — it's how Noticed works. If cost becomes a concern, optimize the LLM call (shorter prompts, caching), don't degrade quality.
...
1. **Remove `analysis_depth` setting**
   ...
   - `/noticed:settings` skill: remove the depth option
   - `noticed/skills/noticed/SKILL.md`: remove `--deep` / `--quick` flags from Step 0

2. **Update session-analyzer agent prompt** (`noticed/agents/session-analyzer.md`)
...
**Acceptance criteria:** Re-run `/noticed` on tacoma after changes — generic workflow proposals should be filtered out by the LLM.
...
**Priority:** HIGH — Noticed runs 4 hooks every session but users can't tell. Without this, `/noticed` feels manual.
**Goal:** High-confidence proposals surface at session start without running `/noticed`. Users always know Noticed is watching.
**Impacts:** UX
...
Currently emits: `{"systemMessage": "Noticed: 3 pending proposals. Run /noticed to review."}`
...
**File:** `noticed/scripts/check-pending.py`
**Changes:**
...
- If proactive and high-confidence proposals exist:
  ```json
  {"systemMessage": "Noticed has a high-confidence suggestion based on 6 sessions:\n\n**Add rule: always use vitest, not jest** — you've corrected this 8 times across 6 sessions.\n\nApprove this? Or run `/noticed` to review all 3 proposals."}
  ```
- If not proactive or no high-confidence: current behavior (count + "run `/noticed`")
...
**File:** `noticed/scripts/check-pending.py`
**Changes:**
- When there are no proactive proposals and no effectiveness alerts, but Noticed has been active:
  - Count total sessions tracked (from unanalyzed log + applied history)
  - Emit brief health line: `"Noticed: tracking 23 sessions for this project. All artifacts effective."`
...
**Files:** `noticed/scripts/read-settings.py`, `noticed/scripts/write-settings.py`, `noticed/skills/settings/SKILL.md`
**Changes:**
- Add `proactive_proposals` to settings defaults (default: `true`)
- Add to `/noticed:settings` skill: "Proactive proposals: surface high-confidence suggestions at session start (default: on)"
- `check-pending.py` reads this setting before deciding what to emit

#### Step 5: Product framing

**Files:** `noticed/README.md`, `README.md`
**Changes:**
- Update "How it works" section to say: "Noticed watches every session automatically. It surfaces high-confidence findings at session start. Run `/noticed` anytime to review all proposals."
- Add "How Noticed learns" section: brief explanation of the feedback loop (dismiss → calibrate → better proposals)
...
**Acceptance criteria:** Session start shows a meaningful Noticed message that reflects actual system state. High-confidence proposals can be approved without `/noticed`.
...
- After: `"Noticed has 3 proposals. Run /noticed to review."` (clean, reads well in the terminal notification line)
...
**File:** `noticed/scripts/format-proposals.py`
**Changes:**
- Accept optional `previous_proposal_ids` list in input JSON (IDs from last `/noticed` run)
...
**File:** `noticed/scripts/cache-manager.py`
**Changes:**
...
**File:** `noticed/skills/noticed/SKILL.md`
**Changes:**
...
**Acceptance criteria:** Running `/noticed` twice shows a "what changed" summary on the second run. Evidence is readable without truncation cutting off key info.
...
**Files:** `noticed/scripts/build-proposals.py`, `noticed/scripts/finalize-proposals.py`, `noticed/scripts/cache-manager.py`
**Changes:**
...
#### Step 2: `/noticed --diagnose`

**New file:** `noticed/scripts/diagnose.py`
**File:** `noticed/skills/noticed/SKILL.md`
**Changes to SKILL.md:**
- Add to Step 0: if user invoked `/noticed --diagnose`, run `diagnose.py` and show output instead of normal flow
...
- Run `mypy noticed/scripts/` and fix warnings
...
**Goal:** Users can trace why any Noticed-generated artifact exists.
**Impacts:** UX
...
**File:** `noticed/skills/noticed/SKILL.md`
**Changes:**
- Add to Step 0: if user invoked `/noticed --explain <path>`, run explain flow instead of normal analysis
- Explain flow:
  ...
  4. If not found: "This artifact wasn't created by Noticed, or predates Noticed's tracking."
...
**Acceptance criteria:** `/noticed --explain .claude/rules/use-vitest.md` shows the original evidence and proposal.
...
4. Run `/noticed` — verify filtered proposals replace raw script proposals in output
5. Document any bugs and fix

**Files changed:** Bug fixes only — no planned structural changes.

**Acceptance criteria:** `/noticed` shows LLM-filtered proposals when deep cache exists. Generic workflow proposals are filtered out.
...
- Anthropic's official guidance: ~200 lines for CLAUDE.md (upper bound ~500). No specific limit for individual rule files. Noticed's 50-100 line rule budget is our convention, not Anthropic's.
...
**Goal:** Detect session health patterns from transcript data and generate fully actionable proposals — Noticed reads the files, drafts the rules, and presents them for approval. Opt-in deep mode with higher LLM budget, justified by long-term token savings.
**Impacts:** UX, Accuracy

**Relationship to 5.2 Self-Cost Tracking:** 5.2 is introspection ("how much does Noticed cost?"). P9 is user-facing value ("how can your sessions be more effective?"). They are orthogonal.

**Background:**
Recent Anthropic staff guidance (May 2026) positions session management as the primary lever for Claude Code effectiveness. Key points: context rot degrades performance around ~300-400k tokens; starting fresh sessions for new tasks is critical; rewind is preferred over correction; subagents are a context management tool. Claude Code's `/usage` command shows token consumption with generic tips. Noticed can detect project-specific patterns and generate concrete artifacts to address them.

**Core principle:** Informational signals without actionable proposals don't meet Noticed's bar. Noticed's value is in doing the work: analyze, propose, apply. P9 must generate fully drafted artifacts the user can approve — not dashboards the user has to interpret.

**Positioning:** `/usage` says "you're using a lot of tokens." Generic guidance says "start new sessions for new tasks." Noticed says "Claude reads `src/config.ts` in 87% of your sessions — here's a scoped rule summarizing its key facts so Claude doesn't need to re-read it. Approve?"
...
- **Session length (turn count):** No automatable remedy — Noticed can't make a user start new sessions.
...
**File:** `noticed/scripts/analyze-transcripts.py`
**Changes:**
...
**New setting:** `session_health_analysis: true/false` (default: `false`). Configured via `/noticed:settings`.

**File:** `noticed/scripts/background-analyze.py`
**Changes:**
...
**File:** `noticed/scripts/build-proposals.py`
**Changes:**
...
**File:** `noticed/skills/settings/SKILL.md`
**Changes:**
...
**File:** `noticed/scripts/read-settings.py`, `noticed/scripts/write-settings.py`
**Changes:**
...
**Version bump:** Required (changes to files under `noticed/`).
...
1. Enable `session_health_analysis` on 2-3 real projects with established session history (Noticed repo, portfolio-site, PriorityAppXcode, or similar).
2. Run background analysis. Wait for `session-health.json` cache to populate.
3. Run `/noticed`. Record every session-health proposal: file targeted, drafted content, decision (approve/modify/skip/dismiss), reason if dismissed.
...
- Deep mode: when `session_health_analysis` is enabled, Noticed reads frequently-accessed files, drafts scoped rules, and presents them as standard proposals in `/noticed`.
...
**Status:** Not started
**Priority:** HIGH — biggest single insight transferred from Designer-Noticed. Verified in `noticed/scripts/check-pending.py` lines 141-153: the proposal-count notification fires every SessionStart whenever `proactive_proposals` is enabled, the deep cache is fresh, and `pending_count > 0` — there is no last-surfaced state. Users see the same "Noticed has N proposals" line on every session start until they either run `/noticed` or dismiss all proposals, which trains them to ignore the notification.
...
**Origin:** Designer-Noticed splits its pipeline at a synthesis boundary — detectors fire on every event, but `AppCore::synthesize_pending()` only runs on track completion (debounced 30s) or first workspace-home view of the day per project. The result is a calmer feed where each notification reflects a genuine change in state. Noticed has all the same primitives (background analysis on SessionStart, pending proposals cache, last-run cache from P2) but lacks the boundary gate.
...
If none fire, `check-pending.py` stays silent even when pending proposals exist. The user can still run `/noticed` manually any time — the cached proposals are not gated, only the notification is.

**Implementation plan:**

#### Step 1: Boundary state tracking

**New file:** `~/.claude/noticed/projects/<hash>/last-surfaced.json`
```json
{
...
#### Step 4: Mark surface on `/noticed` invocation

**File:** `noticed/skills/noticed/SKILL.md`
**Changes:**
- After Step 1 (loading proposals), record a surface event — even if the user runs `/noticed` manually, that counts as having seen the current proposal set. Prevents the next SessionStart from immediately re-surfacing the same IDs.
...
**Files changed:** `check-pending.py`, `read-settings.py`, `write-settings.py`, `noticed/skills/settings/SKILL.md`, `noticed/skills/noticed/SKILL.md`. Version bump required.
...
**Status:** Not started
**Priority:** MEDIUM — incremental quality fix. Designer's per-detector window hash is cleaner than Noticed's current ID-based dedup and catches "same evidence, slightly different proposal text" cases that today produce near-duplicates.
...
**Why this matters for Noticed:** P0 validation found memory promotions duplicating correction-derived proposals (4× `promote-feedback-ios-build` duplicating `ios-simulator-build-rule`). The current ID-based dedup doesn't catch this because the IDs differ. A content-addressed digest does.
...
**File:** `noticed/scripts/build-proposals.py` `build_proposals()` (line 1406)
**Changes:**
...
**File:** `noticed/scripts/finalize-proposals.py`
**Changes:**
...
- Re-running `/noticed` produces the same digests for unchanged evidence.
...
**Status:** Not started
**Priority:** MEDIUM — fills two real gaps in Noticed's proposal taxonomy that Designer-Noticed exposed.
**Goal:** Two new builders covering pruning (`type: "archive"` — non-destructive move to `.claude/noticed/archive/`) and contradiction detection (`origin: "conflict_resolution"`). The archive builder replaces an earlier "removal" design that would have required expanding the destructive-operations security rule.
**Impacts:** Accuracy, completeness

**Origin:** Designer's `ProposalKind` enum includes `RemovalCandidate`, `Demotion`, and `ConflictResolution`. Noticed today has demotion only — and demotion fires only when CLAUDE.md is over-budget. Stale rules with low reference rate get *flagged* (staleness builder) but never proactively *removed*. And no builder detects when two rules contradict each other.
...
**Why archive, not delete:** The security rule (`.claude/rules/security.md`) lists exactly two destructive-operation exceptions, both narrow mechanical migrations of Noticed-owned artifacts moving between known locations (legacy `commands/` → skills, legacy `.claude/noticed/` → user-level storage). Heuristic-driven proactive removal based on staleness thresholds is a categorically different exception — proactive, not migratory; threshold-driven, not location-driven. Adding it would be a substantive policy expansion that deserves its own decision, not a checkbox in a planning PR.

The archive design sidesteps the security rule entirely. Approving an archive proposal **moves** the file from its current location to `.claude/noticed/archive/<original-relative-path>.md`, preserving the content and the original path-in-name. This is a write operation (within the already-permitted `.claude/noticed/` boundary) plus a delete-after-copy of the source file. Per the security rule's two existing exceptions, copy-then-delete migrations of Noticed-owned artifacts are already in scope. The user can manually delete the archive directory later if they want; Noticed never does.
...
- **Noticed-generated only.** Path must appear in `history/applied.json`. Noticed never proposes archival of user-authored content.
...
- **Reversible.** The file is in `.claude/noticed/archive/` — `git mv` it back if approval was a mistake.
...
- Handle `type: "archive"` in the apply path:
  ...
  2. Compute destination: `.claude/noticed/archive/<rel-path-from-claude-dir>` (e.g., `.claude/rules/foo.md` → `.claude/noticed/archive/rules/foo.md`).
...
**File:** `noticed/skills/noticed/SKILL.md`
**Changes:**
- New section for archive proposals: show full content preview, require type-to-confirm filename input. Don't bundle archive proposals with other proposals in the same AskUserQuestion — present one at a time. Include a one-line note: "Files are moved to `.claude/noticed/archive/`, not deleted. Restore with `git mv` or by editing the archive directory."

**File:** `.claude/rules/security.md`
**Changes:**
- No new exception needed. The archive flow is copy-then-delete within the existing `.claude/noticed/` write boundary, structurally identical to the legacy-storage migration exception (#2) already permitted.
- Add a short clarifying note under "Destructive operations" that "copy-then-delete migration of Noticed-generated artifacts into `.claude/noticed/archive/` is permitted under the same logic as the legacy-storage migration."

#### Step 2: Conflict resolution builder

**File:** `noticed/agents/session-analyzer.md`
**Changes:**
...
- `test_archive_apply_moves_file_to_archive_dir` — file ends up in `.claude/noticed/archive/`
- `test_archive_apply_preserves_relative_path` — `.claude/rules/foo.md` → `.claude/noticed/archive/rules/foo.md`
...
**Files changed:** `build-proposals.py`, `finalize-proposals.py`, `format-proposals.py`, `noticed/agents/session-analyzer.md`, `noticed/skills/noticed/SKILL.md`, `.claude/rules/security.md` (clarifying note only — no new exception). Version bump required.

**Acceptance criteria:**
- Archive proposals only target Noticed-generated artifacts.
...
- Archived files end up in `.claude/noticed/archive/` preserving original relative path; original content also snapshot in `history/applied.json`.
...
**Priority:** SKIP unless Designer ships a learning layer that overlaps with Noticed's detectors.
**Goal:** When Designer is installed and emitting findings for overlapping detectors, Noticed defers to avoid double-surfacing.

**Origin:** Designer-Noticed includes a Noticed probe (`if ~/.claude/plugins/noticed/ exists, disable overlapping detectors`). Symmetric: Noticed could probe for Designer. Not worth implementing speculatively.

If revisited: probe `~/.designer/` or whatever Designer's install path becomes, read its detector manifest, suppress matching Noticed builders for that project. ~20 LOC + 3 tests.

---

## Native Build Possibilities

The work on Designer-Noticed surfaced architectural patterns that Noticed can't fully adopt as a plugin but should consider if Noticed ever moves into a native runtime. These possibilities and migration considerations are documented in `core-docs/native-build-possibilities.md`.
...
| Task | Status | Notes |
|------|--------|-------|
| 4.0-prereq Storage split | ✅ Done | v0.3.6 — feedback data to `.claude/noticed/`, personal data stays `~/.claude/noticed/` |
...
| 4.3 Reliability & error visibility | ❌ P3 | Schema validation, /noticed --diagnose, mypy |
...
| 4.5 Explain mode | ❌ P5 | /noticed --explain with evidence trail |
...
**Date:** 2026-04-03
Feedback data (dismissed.json, history/applied.json, feedback_signals.json) moves to `.claude/noticed/` (project-level, git-tracked). Personal settings, cache, pending proposals stay in `~/.claude/noticed/`. 18 new tests (399 total).
...
## Backlog
- `noticed:cleanup` command — detect and remove orphaned `~/.claude/noticed/projects/<hash>/` directories
